...
import { OLLAMA_MODELS, useProviderStore } from '../store/provider.store';
...
export const ModelSettings = () => {
    const { selectedModel, ollamaBaseUrl, setSelectedModel, setOllamaBaseUrl } = useProviderStore();
    ...
    const handleVerify = async () => {
        ...
        try {
            ...
            if (!found) {
                throw new Error(
                    `Modelo "${selectedModel}" não encontrado no Ollama. Disponíveis: ${models.join(', ') || '(nenhum)'}`
                );
            }
            setVerifyStatus('ok');
        } catch (err) {
    ...
    return (
        <div className="flex flex-col gap-6">
            <div className="flex flex-col">
                ...
                <p className="text-muted-foreground text-xs">
                    Selecione o modelo ÍRIS para responder perguntas sobre seguros.
                </p>
            </div>
